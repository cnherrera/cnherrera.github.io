---
layout: default
title: Research Projects & Code Portfolio
description: Scientific software, data workflows, and collaborative research projects
---

# Research Projects & Code Portfolio

This page presents a selection of research projects, software developments, and collaborative work related to data workflows, materials science, and astrophysics.

My work focuses on structuring scientific data, developing reproducible workflows, and supporting collaboration across multidisciplinary research environments.

---

## 🔬 Materials Science & Data Workflows (CEA / BIG-MAP / DIADEM)

### FAIR Data & Standardization

- 🔗 [Segmented Tomography NeXus Format](https://github.com/cnherrera/segmented-tomo-nxs)  
  Development of standardized FAIR data formats for laboratory tomography data based on NeXus structures.  
  → Supports interoperability and data sharing across the French research community.

---

### Ontologies & Knowledge Structuring

- 🔗 [Exa-AToW Ontology](https://github.com/cnherrera/Exa-AToW_onto)  
  Ontology design for structuring materials science workflows, HPC environments, and project ecosystems.  
  → Contributes to improving collaboration, visibility of competencies, and knowledge organization across projects.

---

### Data Workflows & Automation

- 🔗 [Automated Data Calibration – Mantid](https://github.com/cnherrera/automated-data-calibration-d2b-mantid)  
  Automated workflows for powder diffraction data reduction (D2B, ILL) using Mantid (Python).  
  → Developed in the context of large-scale experimental facilities and collaborative projects.

---

## 🔭 Astronomy & Scientific Data Processing

- 🔗 [ALMA Data Reduction Workflows](https://github.com/PhangsTeam/TP_ALMA_data_reduction)  
  Python workflows for calibration, reduction, and analysis of radio astronomy data.  
  → Used in international collaborations for large observational datasets.

- 🔗 [GMC–HII Region Statistical Analysis](https://github.com/cnherrera/GMCHIIR)
  Python tools for spatial matching and statistical analysis of molecular clouds (GMCs) and HII regions in nearby galaxies, built on PHANGS-ALMA and PHANGS-MUSE survey data.
  → Published as part of a study of star formation environments across multiple galaxies.

  
--

## 🤝 Collaborative Research Context

These projects were developed within:

- European collaborative projects (e.g. BIG-MAP)
- National research programs (PEPR NUMPEX, DIADEM)
- International research infrastructures (ILL, IRAM, ALMA)

They reflect a strong focus on:

- Structuring scientific data and workflows  
- Enabling collaboration across multidisciplinary teams  
- Supporting reproducibility and knowledge sharing  
- Contributing to the emergence of interoperable research ecosystems  

---

## 📫 Contact
Cinthya N. Herrera
- LinkedIn: https://www.linkedin.com/in/cinthyaherrera 
- ORCID: https://orcid.org/0000-0001-6405-0785

---
